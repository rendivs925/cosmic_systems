...
for func_test in "${functions[@]}"; do
    ...
    # Run benchmark focusing on the specific function category
    timeout 20s cargo run --release --features parallel,simd --quiet > "$RESULTS_DIR/${func_name}.log" 2>&1 &
    pid=$!
    wait $pid 2>/dev/null || true

    # Extract function-specific metrics (would need app instrumentation)
    fps=$(grep -o "FPS: [0-9.]*" "$RESULTS_DIR/${func_name}.log" | awk '{sum+=$2; count++} END {print sum/count}' 2>/dev/null || echo "0")
    math_time=$(grep -o "Math time: [0-9.]*ms" "$RESULTS_DIR/${func_name}.log" | awk '{sum+=$3; count++} END {print sum/count}' 2>/dev/null || echo "0")

    echo "  FPS: ${fps:-0}"
    echo "  Math Time: ${math_time:-0}ms"
    echo ""
done
