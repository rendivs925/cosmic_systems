...
No optional local imagery manifest is enabled in the checked-in configuration.
Rocket Mode uses the repository's global Earth albedo with the resident
preprocessed terrain and surface packages. This keeps startup self-contained;
no network source or site-specific imagery is required.

If a future offline imagery package is added, it must be reviewed, shipped with
its matching manifest, and remain presentation-only. Sparse imagery never
changes DEM elevation, collision, or terrain-query authority.
